...
<script>
	...
	function openBookSortDialog(){
		...
		$("#fm").form("load",row);
		url="${z:u('admin/type/edit')}?id="+row.id;
		alert(url);
	}
	function deleteBookSort(){
		...
		$.messager.confirm("系统提示","您确认要删掉这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("${z:u('admin/type/delete')}",{ids:ids},function(result){
					if(result.status>0){
...
<body style="margin: 5px;">
	<table id="dg" title="" class="easyui-datagrid" fitColumns="true"
	 pagination="true" rownumbers="true" url="${z:u('admin/type/list')}" fit="true" toolbar="#tb">
		<thead>
			<tr>
				<th field="id" checkbox="true">ID</th>
				<th field="name" width="100" align="center">类型名</th>
			</tr>
		</thead>
	</table>
